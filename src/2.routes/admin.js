...
const uploadImg = utils.multer.uploadProductImg;
...
router.post(
  "/product",
  checkToken,
  uploadImg.fields([{ name: "img_produk" }]),
  product.addProduk
);
...
router.post(
  "/inspections",
  checkToken,
  uploadImg.single("img_inspeksi"),
  productInspections.updateInspections
);
router.get("/inspections", checkToken, productInspections.getInspections);
router.get(
  "/inspections-items",
  checkToken,
  productInspections.getItemInspections
);
